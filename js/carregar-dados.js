...
const totalEconomiaEl = document.getElementById("total-economia");
...
// ESTADO
// ===============================
let carrinho = JSON.parse(localStorage.getItem('carrinhoFlorybalOsorio')) || [];
...
function atualizarStorage() {
    localStorage.setItem('carrinhoFlorybalOsorio', JSON.stringify(carrinho));
    atualizarBadge();
}
...
function calcularEconomia() {
    return carrinho.reduce((acc, item) => {
        if (!item.preco || !item.desconto) return acc;
        const precoFinal = parseFloat(item.desconto.replace("R$ ", "").replace(",", "."));
        if (item.preco > precoFinal) {
            return acc + (item.preco - precoFinal);
        }
        return acc;
    }, 0);
}
...
function mostrarCarrinho() {
    ...
    agrupados.forEach((item, idx) => {
        ...
        div.innerHTML = `
            <div class="item-carrinho-info">
                ...
                <small>${item.desconto}${item.quantidade > 1 ? ` × ${item.quantidade}` : ''}</small>
                ${item.quantidade > 1 ? `<small>Subtotal: ${subtotal}</small>` : ''}
            </div>
    ...
    if (totalEconomiaEl) {
        totalEconomiaEl.textContent = `Você economiza: ${formatarPreco(calcularEconomia())}`;
        totalEconomiaEl.style.display = temItens ? "block" : "none";
    }
...
fetch('./dados-produtos.json')
    ...
    .then(dados => {
        ...
        for (let cat in dados) {
            ...
            titulo.className = 'catalogo__categoria-titulo';
        ...
        dados[cat].itens.forEach(item => {
            ...
            const precoFinal = temDesconto
                ? formatarPreco(item.desconto)
                : precoOriginal;
            const descontoValor = temDesconto
            ...
            card.innerHTML = `
                    ...
                    <div class="card__body">
                        ...
                        ${item.obs ? `<span class="obs-produto">${item.obs}</span>` : ''}
                        <div class="card__prices">
                            ${temDesconto
                        ? `<span class="preco-riscado">De ${precoOriginal}</span>
                                   <div class="price-row">
                                        <span class="price-tag">${precoFinal}</span>
                                        <span class="discount-pill">Economize ${descontoValor}</span>
                                   </div>`
                        : `<div class="price-row">
                                <span class="price-tag">${precoOriginal}</span>
                           </div>`
                    }
                        </div>
...
btnFinalizar.onclick = () => {
    ...
    window.open(`https://wa.me/5554994065076?text=${encodeURIComponent(msg)}`);
    carrinho = [];
    atualizarStorage();
    mostrarCarrinho();
...
if (carousel) {
    const slidesContainer = carousel.querySelector('.hero__slides');
